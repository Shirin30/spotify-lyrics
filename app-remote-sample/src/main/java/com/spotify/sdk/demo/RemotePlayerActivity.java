...
public class RemotePlayerActivity extends FragmentActivity {
  ...
  /** URL for Lyrics data from the Musixmatch dataset */
  private static final String API_ID =
          "f8ba91e753e9fa2f7d2115f3fc733882";
  ...
  private final Subscription.EventCallback<PlayerState> mPlayerStateEventCallback =
      new Subscription.EventCallback<PlayerState>() {
        ...
        public void onEvent(PlayerState playerState) {
          ...
          String url=String.format("https://api.musixmatch.com/ws/1.1/matcher.lyrics.get?format=jsonp&callback=callback&q_track=%s&q_artist=%s&apikey=%s",trackName,Artist,API_ID);

          TextView textView = (TextView) findViewById(R.id.textview);
          new GetLyricsTask(textView).execute(url);
  ...
  private class GetLyricsTask extends AsyncTask<String, Void, String> {
    ...
    public GetLyricsTask(TextView textView) {
      this.textView = textView;
    }

    @Override
    protected String doInBackground(String... strings) {
      String lyricsBody = "UNDEFINED";
      try {
        ...
        string = string.substring(9, string.length()-2);

        JSONObject xyz = new JSONObject(string);
        JSONObject message=xyz.getJSONObject("message");
    ...
    protected void onPostExecute(String lyricsBody) {
      textView.setText("Lyrics of the current song: " + lyricsBody);
    }
  }
